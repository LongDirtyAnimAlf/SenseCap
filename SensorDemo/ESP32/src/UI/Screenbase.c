...
void BaseScreenSetup(lv_obj_t * basescreen, lv_event_cb_t event_cb_more)
{
  ...
  lv_obj_move_to_index(basescreeninfo, 0);
}
...
{
  lv_obj_t * nav = lv_obj_get_child(scr, 0);
  lv_obj_t * obj = lv_obj_get_child(nav, 0);
  if(!lv_obj_check_type(obj, &lv_label_class)) obj = NULL;
  return obj;    
}
